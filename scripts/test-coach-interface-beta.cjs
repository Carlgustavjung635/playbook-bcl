...
await page.evaluate(()=>{
  ...
  state.section='home';state.view=null;state.gages=[];state.gageDraws=[];state.ardoiseAssignments=[];state.pintadePeriods=[];state.pintadeRequests=[];state.pintadeIncidents=[];state.challenges=[];state.convocs=[];
...
await page.screenshot({path:path.join(output,'coach-beta-252-home.png')});
await page.getByRole('button',{name:'A Planning',exact:true}).click().catch(()=>page.locator('.bottom-nav button').filter({hasText:'Planning'}).click());
...
await page.getByRole('button',{name:'+ Nouveau',exact:true}).click();assert.ok(await page.locator('#modal-root input').count());await page.evaluate(()=>closeModal());await page.waitForTimeout(120);ok('Planning connecté au vrai formulaire de création d’événement');
await page.locator('.bottom-nav button').filter({hasText:'Basket'}).click();assert.equal(await page.locator('.coach-beta-basket').count(),1);
...
await page.locator('.bottom-nav button').filter({hasText:'Matchs'}).click();assert.ok(await page.locator('[data-beta-action="ffbb"]').count());ok('Matchs conserve l’accès au championnat FFBB');
await page.locator('.bottom-nav button').filter({hasText:'Équipe'}).click();assert.equal(await page.locator('.coach-beta-team').count(),1);
...
await page.locator('.bottom-nav button').filter({hasText:'Accueil'}).click();await page.locator('[data-beta-action="legacy"]').click();assert.equal(await page.locator('.coach-beta-home').count(),0);assert.equal(await page.locator('.coach-beta-banner').count(),1);ok('Accueil complet historique disponible sans désactiver la bêta');
...
fs.writeFileSync(path.join(output,'verification-coach-beta-252.json'),JSON.stringify({passed,errors,syntheticData:true,remoteBusinessWrites:false},null,2));
